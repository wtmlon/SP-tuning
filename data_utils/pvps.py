...
class PVP(ABC):
    ...
    def encode(self, example: InputExample, priming: bool = False, labeled: bool = False, aug: bool = False, seed: int = 13) \
        ...
        if aug and aug_parts_a:
            aug_parts_a = [x if isinstance(x, tuple) else (x, False)
                       for x in aug_parts_a]
            aug_parts_a = [(tokenizer.encode(x, add_special_tokens=False, **kwargs), s)
                       for x, s in aug_parts_a if x]
            self.truncate(
                aug_parts_a, parts_b, max_length=self.wrapper.config.max_seq_length - num_special)
            tokens_aug = [token_id for part,
...
class ColaPVP(PVP):
    ...
    def get_parts(self, example: InputExample) -> FilledPattern:
        ...
        aug_string_list_a = ['Is', 'that', self.mask, '?', text_a]
        string_list_b = []
        block_flag_a = self.BLOCK_FLAG
        block_flag_b = []
        assert len(string_list_a) == len(block_flag_a)
...
class TrecPVP(PVP):
    ...
    def get_parts(self, example: InputExample) -> FilledPattern:
        ...
        aug_string_list_a = [text_a, 'Full', self.mask, '.']
        string_list_b = []
        block_flag_a = self.BLOCK_FLAG
        block_flag_b = []
        assert len(string_list_a) == len(block_flag_a)
...
class SnliPVP(PVP):
    ...
    PATTERN = ['text_a', '.', 'self.mask', ',', ' it', ' looks', 'like', 'text_b'] # 42
...
#     }
#
#     #PATTERN = ['text_a', '?', 'self.mask', ',', ' I', ' was', ' wondering', 'text_b']
#     PATTERN = ['text_a', '?', 'self.mask', ',', ' no', ',', 'text_b']
#
#     BLOCK_FLAG = [0, 1, 0, 1, 1, 1, 0]
#
#     def get_parts(self, example: InputExample, seed: bool = False) -> FilledPattern:
#         text_a = self.shortenable(example.text_a)
#         text_b = self.shortenable(example.text_b)
...
#         # few-shot
#         #string_list_a = [text_a, '?', self.mask, ',', text_b]
#         #string_list_a = [text_a, '?', self.mask, ',', 'I', 'was', 'wondering', text_b]
#         #aug_string_list_a = [text_a, '?', self.mask, ',', 'it', 'is', ',', text_b]
#         string_list_a = [text_a, '?', self.mask, ',', 'no', ',', text_b]
#         aug_string_list_a = [text_a, '?', self.mask, ',', 'however', ',', text_b]
#         string_list_b = []
#         block_flag_a = self.BLOCK_FLAG
#         block_flag_b = []
#         assert len(string_list_a) == len(block_flag_a)
